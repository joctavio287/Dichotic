...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    text_instructions = visual.TextStim(win=win, name='text_instructions',
        text='En este experimento se repetirán varios ensayos en los cuales vas a tener que escuchar diferentes historias. \n\nPonete comodo/a, intenta no mover mucho el cuerpo y tratá de fijar la mirada en la cruz que aparecerá en el centro de la pantalla.\n\nEn cada ensayo se presentaran dos historias, una en cada oreja. La idea es que prestes atención únicamente a la historia que se te indique en cada ensayo. Luego, habrá unas preguntas de comprensión que deberás responder con el teclado.\n\nCuando estes listo/a apretá la barra espaciadora para continuar...\n\n',
        font='Arial',
    ...
    fixation_listening = visual.ShapeStim(
        win=win, name='fixation_listening', vertices='cross',
        size=(0.1, 0.1),
        ori=0.0, pos=(0, 0), draggable=False, anchor='center',
        lineWidth=1.0,
        colorSpace='rgb', lineColor='white', fillColor='white',
    ...
    for thisCondition_trial in condition_trials:
        ...
        ConditionPrompt = data.Routine(
            ...
            components=[text, key_resp_prompt],
        )
        ...
        text.setText(f"Presta atención a la historia del lado {lado}.\n\n\nPresioná la barra espaciadora para continuar...")
        ...
        while continueRoutine:
            ...
            if text.status == STARTED:
                ...
                if tThisFlipGlobal > text.tStartRefresh + key_resp_prompt.status == FINISHED-frameTolerance:
                    # keep track of stop time/frame for later
        ...
        for thisTrial in trials:
            ...
            continueRoutine = True
            # update component parameters for each repeat
            bip.setSound('../data/bip.wav', hamming=True)
            ...
            while continueRoutine:
                ...
                if fixation_bip.status == STARTED:
                    # is it time to stop? (based on global clock, using actual start)
                    if tThisFlipGlobal > fixation_bip.tStartRefresh + bip.status == FINISHED-frameTolerance:
                        # keep track of stop time/frame for later
        ...
        Listening = data.Routine(
            name='Listening',
            components=[audiobook, fixation_listening],
        )
        Listening.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        # Run 'Begin Routine' code from audiobook_code
        audiobooks_possible_combinations = audiobook_combinations[audiobook_combinations['condition_label']==condition_label]
        ear = 'L' if target=='Left' else 'R'
        n_story = trials.thisRepN*2-1
        if condition_label[-2:]=='AB':
            audiobook_df = audiobooks_possible_combinations[audiobooks_possible_combinations['number_story_A']==n_story]
            audiofile_path = audiobook_df['filename']
        elif condition_label[-2:]=='BA':
            audiobook_df = audiobooks_possible_combinations[audiobooks_possible_combinations['number_story_A']==n_story+1]
            audiofile_path = audiobook_df['filename']
        
        if condition_label.split('F')[-1][0]==ear:
            attended_story = audiobook_df['number_story_A']
        else:
            attended_story = audiobook_df['number_story_B']
        questionary = audiobook_questionary.iloc[n_story-1]
        print(questionary.head())
        # Add to log
        thisExp.addData('audiofile_path', audiofile_path)
        ...
        audiobook.setSound(audiofile_path, hamming=True)
        ...
        while continueRoutine:
            ...
            if audiobook.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
            ...
            # if fixation_listening is starting this frame...
            if fixation_listening.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                fixation_listening.frameNStart = frameN  # exact frame index
                fixation_listening.tStart = t  # local t and not account for scr refresh
                fixation_listening.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(fixation_listening, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'fixation_listening.started')
                # update status
                fixation_listening.status = STARTED
                fixation_listening.setAutoDraw(True)
            
            # if fixation_listening is active this frame...
            if fixation_listening.status == STARTED:
                # update params
                pass
            
            # if fixation_listening is stopping this frame...
            if fixation_listening.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > fixation_listening.tStartRefresh + audiobook.status == FINISHED-frameTolerance:
                    # keep track of stop time/frame for later
                    fixation_listening.tStop = t  # not accounting for scr refresh
                    fixation_listening.tStopRefresh = tThisFlipGlobal  # on global time
                    fixation_listening.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'fixation_listening.stopped')
                    # update status
                    fixation_listening.status = FINISHED
        ...
        for thisQuestions_trial in questions_trial:
            ...
            question = questions[questions_trial.thisRepN]
            correct_answer = correct_answers[questions_trial.thisRepN]
            answer_a = answers_a[questions_trial.thisRepN]
            answer_b = answers_b[questions_trial.thisRepN]
            answer_c = answers_c[questions_trial.thisRepN]
    ...
    # completed 7.0 repeats of 'condition_trials'
